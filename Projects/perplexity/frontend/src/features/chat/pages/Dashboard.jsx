            ...
            <div className="md">
              <ReactMarkdown
                ...
                components={{
                  ...
                  blockquote: ({ children }) => <blockquote className="bq">{children}</blockquote>,
                  strong: ({ children }) => <strong style={{ fontWeight: 600 }}>{children}</strong>,
                  a: ({ href, children }) => (
                    <a href={href} target="_blank" rel="noopener noreferrer" className="ml">{children}</a>
                  ),
                  h1: ({ children }) => <h1 style={{ fontSize: "1.18em", fontWeight: 700, margin: ".55em 0 .28em" }}>{children}</h1>,
                  h2: ({ children }) => <h2 style={{ fontSize: "1.07em", fontWeight: 600, margin: ".5em 0 .25em" }}>{children}</h2>,
                  h3: ({ children }) => <h3 style={{ fontSize: "1em", fontWeight: 600, margin: ".45em 0 .22em" }}>{children}</h3>,
                  table: ({ children }) => (
        ...
        {!isUser && !msg.isStreaming && (
          <div className="acts">
            <button
              ...
              onClick={() => onFeedback({ chatId, messageId: msg.id, feedback: "like" })}
              title="Helpful"
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill={msg.feedback === "like" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
              </svg>
            </button>
            <button
              className={`abt ${msg.feedback === "dislike" ? "abt-on" : ""}`}
              onClick={() => onFeedback({ chatId, messageId: msg.id, feedback: "dislike" })}
              title="Not helpful"
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill={msg.feedback === "dislike" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
  ...
  return (
    <div className="overlay" onClick={onCancel}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="modal-p"><em>"{title}"</em> will be permanently removed.</p>
        <div className="modal-row">
          <button className="modal-no" onClick={onCancel}>Cancel</button>
          <button className="modal-yes" onClick={onOk}>Delete</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="um-wrap" ref={ref}>
      {open && (
        <div className="um-popup">
          ...
          <button className="um-item" onClick={() => { onToggleTheme(); setOpen(false); }}>
            {isDark ? (
          ...
          <div className="um-divider" />
          <button className="um-item um-out" onClick={() => { onLogout(); setOpen(false); }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <button className="um-btn" onClick={() => setOpen((o) => !o)}>
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink: 0, opacity: 0.4 }}>
...
const Dashboard = () => {
  ...
  return (
    <div className={`root ${isDark ? "dark" : "light"}`}>
      ...
      {/* ── SIDEBAR ────────────────────────────────────────────────── */}
      {/* ✅ FIX: No inline style. Only className controls open/close. */}
      <aside className={`sb${sidebarOpen ? " sb-open" : ""}`}>
        <div className="sb-top">
          ...
          <button className="ib" onClick={() => setSidebarOpen(false)} title="Collapse">
          ...
          {sorted.length === 0 ? (
            ...
            sorted.map((chat) => (
              ...
              >
                ...
                >
                  {loadingId === chat.id ? (
                    <svg className="spin-s" width="11" height="11" viewBox="0 0 24 24" fill="none">
                      <circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="2.5" strokeOpacity=".25" />
                      <path d="M12 3a9 9 0 019 9" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
                    </svg>
                  ) : (
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink: 0, opacity: 0.35 }}>
        ...
        <header className="topbar">
          ...
          {(busy || loadingId) && (
            <div className="thinking">
              <svg className="spin-s" width="12" height="12" viewBox="0 0 24 24" fill="none">
                <circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="2.5" strokeOpacity=".25" />
                <path d="M12 3a9 9 0 019 9" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
              </svg>
            ...
            <div className="msgs-loading">
              <div className="msgs-loading-inner">
                <svg className="spin" width="22" height="22" viewBox="0 0 24 24" fill="none">
                  <circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="2" strokeOpacity=".2" />
          ...
          <div className="msgs">
            {messages.length === 0 && !busy && !loadingId ? (
              <div className="empty">
                ...
                <p className="empty-p">Ask anything — research, writing, code, analysis.</p>
                <div className="chips">
                  {suggestions.map((s) => (
        ...
        <div className="inp-wrap">
          <form onSubmit={submit} className="inp-box">
            ...
            <div className="inp-foot">
              ...
              <button type="submit" disabled={!input.trim() || busy} className="send">
                {busy ? (
          ...
          <p className="disc">Perplexity can make mistakes — verify important information.</p>
        </div>
      </main>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ═══════════════════════════════════════════════════════════
           SIDEBAR — DESKTOP (width-collapse) + MOBILE (slide)
           The key fix: NO inline style on <aside>. CSS only.
        ═══════════════════════════════════════════════════════════ */

        /* Desktop default: sidebar is visible, collapses by shrinking width */
        .sb {
          flex-shrink: 0;
          background: var(--bg2);
          ...
          overflow: hidden;
          gap: 6px;
          /* Animate width, padding, opacity so it smoothly collapses */
          width: 252px;
          padding: 14px 10px;
          opacity: 1;
          transition:
        ...
        /* Desktop CLOSED state — collapse to zero width */
        .sb:not(.sb-open) {
          width: 0;
          padding-left: 0;
          padding-right: 0;
        ...
        .new-btn {
          ...
          background: var(--ic-bg); border: 1px solid var(--b2); border-radius: 9px;
          padding: 9px 12px; color: var(--t2); font-size: 13px; font-weight: 500;
        ...
        .empty-sb { font-size: 12px; color: var(--t3); text-align: center; padding: 20px 0; white-space: nowrap; }

        .crow { display: flex; align-items: center; gap: 2px; }
        .cb {
          display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0;
          background: none; border: none; cursor: pointer; border-radius: 8px;
          padding: 7px 8px; color: var(--t2); text-align: left;
        ...
        .del-btn {
          flex-shrink: 0; background: none; border: none; cursor: pointer; color: var(--t3);
        ...
        .um-btn {
          ...
          transition: background .14s; text-align: left; white-space: nowrap;
        }
        .um-btn:hover { background: var(--hov); }
        .um-av {
          width: 30px; height: 30px; border-radius: 50%; background: var(--av-bg);
          border: 1px solid var(--b2); color: var(--t); font-size: 11px; font-weight: 700;
          display: flex; align-items: center; justify-content: center; flex-shrink: 0;
        }
        .um-uname {
          flex: 1; font-size: 13px; font-weight: 500; color: var(--t);
        ...
        .um-av-lg {
          width: 36px; height: 36px; border-radius: 50%; background: var(--av-bg);
        ...
        .msgs-loading {
          position: absolute; inset: 0; z-index: 10; background: var(--bg2);
          display: flex; align-items: center; justify-content: center; animation: fi .15s ease;
        }
        .msgs-loading-inner { display: flex; align-items: center; gap: 10px; color: var(--t2); font-size: 13px; }

        .msgs {
          flex: 1; overflow-y: auto; padding: 20px 0 8px; display: flex; flex-direction: column;
        ...
        .empty-icon {
          width: 52px; height: 52px; border-radius: 14px; border: 1px solid var(--b2);
          background: var(--ic-bg); display: flex; align-items: center; justify-content: center;
        ...
        .chip {
          ...
          color: var(--t2); font-size: 13px; padding: 10px 14px; cursor: pointer;
          text-align: left; line-height: 1.4; transition: background .14s, color .14s;
        }
        .chip:hover { background: var(--hov); color: var(--t); }

        .mrow {
          display: flex; gap: 10px; padding: 6px 22px;
        ...
        .bu {
          background: var(--ub); color: var(--ut); border-radius: 16px 16px 3px 16px;
          padding: 10px 14px; font-size: 14px; border: 1px solid var(--b);
        }
        .ba {
          background: var(--ab); color: var(--t); border-radius: 3px 16px 16px 16px;
        ...
        .ic {
          background: var(--ic-bg); padding: 2px 6px; border-radius: 5px;
          font-family: 'SF Mono', 'Fira Code', monospace; font-size: .84em; border: 1px solid var(--b);
        }
        .cb {
          background: var(--cb-bg); border: 1px solid var(--b); border-radius: 10px;
        ...
        .inp-box {
          border: 1.5px solid var(--b2); border-radius: 13px; padding: 12px 14px 10px;
        ...
        .send {
          background: var(--sbg); color: var(--sfg); border: none; border-radius: 8px;
        ...
        .overlay {
          ...
          z-index: 100; animation: fi .2s ease;
        }
        .modal {
        ...
        .modal-icon {
          width: 44px; height: 44px; border-radius: 12px; background: rgba(239,68,68,.1);
        ...
        .modal-no {
          flex: 1; background: var(--ic-bg); border: 1px solid var(--b2); border-radius: 9px;
        ...
        .modal-yes {
          flex: 1; background: #ef4444; border: none; border-radius: 9px; color: #fff;
        ...
        /* ═══════════════════════════════════════════════════════════
           MOBILE — sidebar slides in/out from the left
           Overrides the desktop width-collapse approach
        ═══════════════════════════════════════════════════════════ */
        .sb-backdrop { display: none; }

        @media (max-width: 640px) {
          /* Reset desktop collapse and use transform instead */
          .sb {
            position: fixed;
            top: 0; left: 0;
            height: 100dvh;
